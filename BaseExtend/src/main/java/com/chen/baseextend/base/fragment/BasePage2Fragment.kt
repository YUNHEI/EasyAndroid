 ...
 * @date 2019/3/27
 */
abstract class BasePage2Fragment<T : RootBean> : BaseSimpleFragment() {
...
//    override val contentLayoutId = R.layout.fragment_page_2
    ...
    internal class BasePageAdapter2<T : RootBean>(val fragment: BasePage2Fragment<T>) :
        FragmentStateAdapter(fragment) {

        override fun getItemCount() = fragment.items.size

        override fun createFragment(position: Int) =
            fragment.getFragmentPage(fragment.items[position], position)
    }

    open class BaseItemNavAdapter<T : RootBean>(val fragment: BasePage2Fragment<T>) :
