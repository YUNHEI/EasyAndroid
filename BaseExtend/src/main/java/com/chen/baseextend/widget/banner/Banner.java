...
import com.chen.baseextend.widget.banner.listener.OnBannerListener;
import com.chen.baseextend.widget.banner.loader.ImageLoaderInterface;
import com.chen.baseextend.widget.banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private int mIndicatorSelectedResId = R.drawable.ic_indicator_gray_radius;
    private int mIndicatorUnselectedResId = R.drawable.ic_indicator_white_radius;
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        mIndicatorSelectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_selected, R.drawable.ic_indicator_gray_radius);
        mIndicatorUnselectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_unselected, R.drawable.ic_indicator_white_radius);
        ...
        bannerBackgroundImage = typedArray.getResourceId(R.styleable.Banner_banner_default_image, R.mipmap.ic_no_banner);
        typedArray.recycle();
    }
    ...
    public Banner setAds(List<AdvertBean> ads){
        this.mAdvertBeans = ads;
        this.count = ads.size();
        imageUrls.clear();
        for (AdvertBean ad : ads) {
            imageUrls.add(ad.getImage());
        }
        return this;
    }
    ...
    private void setImageList(List<String> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            String url = null;
            AdvertBean bean = null;
            if (i == 0) {
                url = imagesUrl.get(count - 1);
                bean = mAdvertBeans.get(count - 1);
            } else if (i == count + 1) {
                url = imagesUrl.get(0);
                bean = mAdvertBeans.get(0);
            } else {
                url = imagesUrl.get(i - 1);
                bean = mAdvertBeans.get(i -1);
            }
            imageViews.add(imageView);
            if (imageLoader != null) {
                imageLoader.setInformation(context,bean,imageView);
    ...
    public void scrollToItem(int p) {
        viewPager.setCurrentItem(p + 1);
        handler.removeCallbacks(task);
        handler.postDelayed(task, delayTime + 2000);
    }
