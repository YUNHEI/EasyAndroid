...
public abstract class SelectableAdapter<VH extends RecyclerView.ViewHolder>
    ...
    List<Photo> mPhotoUrls = new ArrayList<>();
    ...
    public SelectableAdapter(ArrayList<String> urls, ArrayList<String> orginalPhotos) {
        if (urls != null) {
            this.mUrls.addAll(urls);
            for (String item : mUrls) {
                if (orginalPhotos.contains(item)) {
                    mPhotoUrls.add(new Photo(0, item,true));
                } else {
                    mPhotoUrls.add(new Photo(0, item));
                }
            }
        }
    ...
    public List<Photo> getCurrentPhoto() {
