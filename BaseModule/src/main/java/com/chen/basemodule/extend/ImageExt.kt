...
fun ImageView.load(
    url: String? = null,
    @DrawableRes place: Int = R.drawable.ic_placeholder,
    isOriginSize: Boolean = false,
    thumPath: String? = null,
    imageWidth: Float = 0f,
    imageHeight: Float = 0f,
    autoAdjust: Boolean = false,
    vararg trans: Transformation<Bitmap>
) {
    ...
    if (FileUtil.isLocalPath(url)) {
        ...
        bitmapWidth =
            Regex(patternWidth, RegexOption.IGNORE_CASE).find(url)?.groupValues?.get(1)?.toFloat()
                ?: 0f
        bitmapHeight =
            Regex(patternHeight, RegexOption.IGNORE_CASE).find(url)?.groupValues?.get(1)?.toFloat()
                ?: 0f
    }
    ...
    if (isOriginSize) {
        ...
        layoutParams.run {
            width = (if (viewMaxWidth > 0) min(
                viewMaxWidth,
                bitmapWidth.times(scale)
            ) else bitmapWidth.times(scale)).toInt()
            height = (if (viewMaxHeight > 0) min(
                viewMaxHeight,
                bitmapHeight.times(scale)
            ) else bitmapHeight.times(scale)).toInt()
        }
        ...
        val builder = if (FileUtil.isLocalPath(url)) manager.load(
            getImageContentUri(
                context,
                url
            )
        ) else manager.load(url)
        builder.centerCrop()
            .apply(options.apply {
                if (trans.isNotEmpty()) {
            ...
            override fun onPreDraw(): Boolean {
                ...
                if (viewWidth > 0 && viewHeight > 0) {
                    ...
                    if ((viewWidth > 0 || viewHeight > 0) && !autoAdjust) {
                        ...
                        desWidth = if (viewMaxWidth > 0) min(
                            viewMaxWidth,
                            bitmapWidth.times(scale)
                        ) else bitmapWidth.times(scale)

                        desHeight = if (viewMaxHeight > 0) min(
                            viewMaxHeight,
                            bitmapHeight.times(scale)
                        ) else bitmapHeight.times(scale)
                    }

                    val imageUrl = if (imageWidth <= 0 && !FileUtil.isLocalPath(url)) {
                        val params =
                            "imageView2/1/w/${desWidth.toInt()}/h/${desHeight.toInt()}/format/jpg/q/75|imageslim"
                    ...
                    manager.load(url.run {
                        if (FileUtil.isLocalPath(url)) getImageContentUri(
                            context,
                            url
                        ) else imageUrl
                    }).run {
                        apply(options.apply {
                            if (autoAdjust && FileUtil.isLocalPath(url)) override(
                                desWidth.toInt(),
                                desHeight.toInt()
                            )
                            if (trans.isNotEmpty()) {
