...
object ScreenUtils {
    ...
    fun getRealHeight(): Int {
        ...
        return DisplayMetrics().run {
            (BaseModuleLoad.context.getSystemService(Context.WINDOW_SERVICE) as WindowManager)
                    .defaultDisplay
                    .getRealMetrics(this)
            heightPixels
        }
    ...
    fun getBottomNavigateHeight(): Int {
        (BaseModuleLoad.context.getSystemService(Context.WINDOW_SERVICE) as WindowManager).run {
    ...
    fun getToolbarPadding(context: Context): Int {
        return BasePreference._NOTCH_HEIGHT.run {
            when {
                this > 0 -> this
                hasNotch(context) -> {
                    val notchHeight = getNotchHeight(context)
                    return if (notchHeight > 0) {
                        BasePreference._NOTCH_HEIGHT = notchHeight
                        notchHeight
                    } else {
                        getStatusHeight(context)
                    }
                }
                else -> getStatusHeight(context)
            }
        }
    }
    ...
    fun getStatusHeight(context: Context? = null): Int {
        ...
        if (BasePreference._STATUS_BAR_HEIGHT > 0) {
            return BasePreference._STATUS_BAR_HEIGHT
        } else {
            Resources.getSystem().run {
                getIdentifier("status_bar_height", "dimen", "android").run {
                    if (this > 0) {
                        val height = getDimensionPixelSize(this)
                        BasePreference._STATUS_BAR_HEIGHT = height
                        return height
                    }
                    if (context is Activity) {
                        context.run {
                            ...
                            if (r.top > 0) {
                                BasePreference._STATUS_BAR_HEIGHT = r.top
                                return r.top
                            }
                        }
                    }
    ...
    fun getKeyBoardHeight(activity: Activity): Int {
        return Rect().run {
            ...
            val softKeyboardHeight = SCREEN_HEIGHT - bottom

            if (softKeyboardHeight > 0) {
                BasePreference._KEYBOARD_HEIGHT = softKeyboardHeight
                softKeyboardHeight
            } else {
                max(BasePreference._KEYBOARD_HEIGHT, 767)
            }
        }
    }
