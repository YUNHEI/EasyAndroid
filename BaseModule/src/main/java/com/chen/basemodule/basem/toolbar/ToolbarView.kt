package com.chen.basemodule.basem.toolbar
...
import com.chen.basemodule.util.ScreenUtils
...
class ToolbarView(context: Context, attrs: AttributeSet? = null) :
    ...
    var isImmerse: Boolean = true
        set(value) {
            field = value
            setPadding(0, if (value) ScreenUtils.getToolbarPadding(context) else 0, 0, 0)
        }
    ...
    override fun onApplyWindowInsets(insets: WindowInsets?): WindowInsets {
        insets?.run {
            setPadding(
                paddingLeft,
                if (isImmerse && paddingTop == 0) this.systemWindowInsetTop else paddingTop,
                paddingRight,
                paddingBottom
            )
        }
    ...
    fun attachToolbar(root: View?, autoAdjust: Boolean = true) {
        if (parent == null) {
            root?.let {
                when (it) {
    ...
    fun center(
        s: String,
        @ColorRes color: Int = if (isLight) textColorLight else textColorDark,
        padRect: Rect = Rect(0, 0, 0, 0),
        ...
        return TextView(context).apply {
            ...
            layoutParams =
                FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, dp2px(toolbarHeight))
                    .apply {
    ...
    fun left(
        ...
        return ImageView(context).apply {
            ...
            layoutParams =
                ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, dp2px(toolbarHeight))
            left(this, onClick)
        }
    }
     ...
     */
    fun divider(
        @DimenRes height: Int = R.dimen.dimen_05,
        @ColorRes color: Int = if (isLight) R.color.gray_e3 else R.color.white
    ) {
        mDivider.run {
    ...
    fun <T : View> extend(
        ...
        @DrawableRes parentBackground: Int? = R.drawable.bg_item_white_ef_line,
        parentSize: Pair<Int, Int>? = null,
        margRect: Rect? = Rect(0, 0, 0, 0)
    ): T {
    ...
    fun <T : View> extend(
        ...
        size: Pair<Int, Int>? = null,
        @DrawableRes background: Int? = null,
        @DrawableRes parentBackground: Int? = null,
        parentSize: Pair<Int, Int>? = null,
        margRect: Rect? = Rect(0, 0, 0, 0)
    ): T {
        return extend(
            inflate(context, layout, null) as T,
            size,
            background,
            parentBackground,
            parentSize,
