...
import com.chen.basemodule.widget.smartrefresh.layout.internal.ArrowDrawable;
...
//        }
    ...
    //用于记录更新时间
    public void setClassicsHeaderKey(String key) {
        this.key = key;

        KEY_LAST_UPDATE_TIME = KEY_LAST_UPDATE_TIME + getContext().getClass().getName() + key;
        mShared = getContext().getSharedPreferences("ClassicsHeader", Context.MODE_PRIVATE);
        long lastTime = mShared.getLong(KEY_LAST_UPDATE_TIME, -1);

        if (lastTime <= 0) {
            setLastUpdateTime(new Date());
        } else {
            mLastTime = new Date(lastTime);
        }
    ...
    public void onStateChanged(@NonNull RefreshLayout refreshLayout, @NonNull RefreshState oldState, @NonNull RefreshState newState) {
        ...
        switch (newState) {
            ...
            case PullDownToRefresh:
                ...
                String dateWithHours = DateUtil.getDateWithHours(mLastTime, true, false, false);
                setLastUpdateText(String.format("最后更新: %s", dateWithHours));
                break;
            case Refreshing:
