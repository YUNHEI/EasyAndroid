...
class SlideListSampleFragment : SingleListFragment<WeatherBean>() {
    ...
    private val mItemTouchHelperCallback by lazy { ItemTouchHelperCallback(mAdapter) }

    private val mItemTouchHelper by lazy { ItemTouchHelper(mItemTouchHelperCallback) }

    override val lManager: RecyclerView.LayoutManager by lazy {
        mItemTouchHelper.attachToRecyclerView(mRecycler)
        SlideLayoutManager(mRecycler, mItemTouchHelper)
    }
