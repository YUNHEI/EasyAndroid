...
public class RouteProcessor extends BaseProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        if (CollectionUtils.isNotEmpty(annotations)) {
            Set<? extends Element> routeElements = roundEnv.getElementsAnnotatedWith(Launch.class);
            try {
    ...
    private void parseRoutes(Set<? extends Element> routeElements) throws IOException {
        if (CollectionUtils.isNotEmpty(routeElements)) {
            ...
            ClassName launchTypeCn = ClassName.get(LaunchType.class);
            ClassName swipeTypeCn = ClassName.get(SwipeType.class);
            ...
            for (Element element : routeElements) {
                ...
                if (types.isSubtype(tm, type_Activity)) {                 // Activity
                    ...
                    routeMeta = new RouteMeta(launch, element, RouteType.ACTIVITY, paramsType);
                    routeMeta.setInjectConfig(injectConfig);
                } else if (types.isSubtype(tm, iProvider)) {         // IProvider
                    logger.info(">>> Found provider route: " + tm.toString() + " <<<");
                    routeMeta = new RouteMeta(launch, element, RouteType.PROVIDER, null);
                } else if (types.isSubtype(tm, type_Service)) {           // Service
                    logger.info(">>> Found service route: " + tm.toString() + " <<<");
                    routeMeta = new RouteMeta(launch, element, RouteType.parse(SERVICE), null);
                ...
                for (RouteMeta routeMeta : groupData) {
                                ...
                                if (types.isSameType(tm, iProvider)) {   // Its implements iProvider interface himself.
                                    ...
                                    loadIntoMethodOfProviderBuilder.addStatement(
                                            "providers.put($S, $T.build($T." + routeMeta.getType() + ", $T.class, $S, $S, null, " + routeMeta.getPriority() + ", " + routeMeta.getExtra() +
                                                    ", $T." + routeMeta.getLaunchType() + ", $T." + routeMeta.getSwipeType() + "))",
                    ...
                    loadIntoMethodOfGroupBuilder.addStatement(
                            "atlas.put($S, $T.build($T." + routeMeta.getType() + ", $T.class, $S, $S, " + (StringUtils.isEmpty(mapBody) ? null : ("new java.util.HashMap<String, Integer>(){{" + mapBodyBuilder.toString() + "}}")) + ", " +
                                    routeMeta.getPriority() + ", " + routeMeta.getExtra() + ", $T." + routeMeta.getLaunchType() + ", $T." + routeMeta.getSwipeType() + "))",
                            routeMeta.getPath(),
                            routeMetaCn,
                            routeTypeCn,
                            className,
